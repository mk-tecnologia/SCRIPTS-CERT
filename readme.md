...
Versão atual dos scripts: **2.5.2** — 2026-08-30. Cada script mostra sua versão e data no cabeçalho e aceita a opção `--version`.
...
scripts-cert-installer --use v2.5.2
...
```bash
bash /tmp/scripts-cert-install.sh --version v2.5.2 --yes
```
...
```powershell
scripts-cert-installer -List
scripts-cert-installer -Version v2.5.2 -Yes
scripts-cert-installer -Use v2.5.2
scripts-cert-installer -Rollback
scripts-cert-installer -Prune
```
...
- Não altera `/etc/hosts` nem pergunta sobre isso em nenhum modo, salvo quando `--add-hosts` é informado explicitamente.
